...
data.merge <- transform(data.merge,
                        ...
                        progressed=factor(is.na(firstprg),
                                           ...
                                           ordered=TRUE),
                        beforeprogression=factor(Date <= firstprg, 
                                                 levels=c(TRUE, FALSE),
                                                 labels=c("Before Progression", "After Progression"),
                                                 ordered=TRUE))

# head(data.merge[, c("beforeprogression", "progressed", "progfree", "firstprg")])
...
p1 <- ggplot(plot.data, aes(x=NSE, group=progressed, color=progressed))
p1 <-p1 + geom_density()
...
p2 <- ggplot(plot.data, aes(y=NSE, x=progressed, color=progressed))
p2 <-p2 + geom_boxplot()
