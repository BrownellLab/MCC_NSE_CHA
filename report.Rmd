...
data.pt <- subset(data.merge, PtNum == 10)
with(data.pt, findclosestdatebefore(Date, firstprg))
...
metadata.cols <- c("id", "mccdxdat", "progfree", "firstprg", "nseprgda", "nse1date", "chrne1da", "chraprgdanew")
...
### ChrA new values quantiles

```{r}
summary(data.merge$NSE)
```

```{r results='asis'}
tbl <- ddply(data.merge, .(beforeprogression), function(x) quantile(x$NSE, probs=c(0, 0.25, 0.5, 0.75, 1), na.rm=TRUE))
colnames(tbl)[1] <- "Progression status"
# tbl <- tbl[1:3, ]
xtbl <- xtable(tbl)
print(xtbl)
```
...
```{r results='asis'}
tbl <- ddply(data.merge, .(beforeprogression), function(x) quantile(x$ChrAn, probs=c(0, 0.25, 0.5, 0.75, 1), na.rm=TRUE))
colnames(tbl)[1] <- "Progression status"
# tbl <- tbl[1:3, ]
xtbl <- xtable(tbl)
print(xtbl)
